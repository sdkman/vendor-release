...
**Path 1 — `PUT /candidates/default`.** The route **already** fetches all Mongo version documents for `(candidate, version)` via `findAllVersionsByCandidateVersion` *before* calling `updateDefaultVersion` (today it only uses `headOption`); reuse that in-scope `versions` Seq for tag propagation — no second query. After the successful Mongo default write, for each fetched doc map platform via `PlatformMapper`, derive distribution via `DistributionMapper` from the doc's vendor (`None` for non-java), and assert `lts` through the State API's append-only `POST /versions/tags`. Append semantics preserve the version's other tags, and the endpoint's built-in move behaviour relocates `lts` off the prior default — so there is no need to look up or clear the old default.

**Path 2 — `POST /versions` with `"default": true`.** Fold the tag into the version dual-write that already happens (`upsertVersionStateApi`). The Mongo `Version` model carries no default flag, so the route computes the tags value from the request — `Some(List(LtsTag))` when `req.default` is true, otherwise `None` — and threads it into the client as a new `tags: Option[List[String]]` parameter. The default→`lts` policy lives in the route; the client stays generic. The State API's `POST /versions` applies *replace* semantics, which correctly moves `lts` off the prior default. Since a non-java version's tag set is only ever `["lts"]` (or empty), replacing it is exactly the desired outcome — there is nothing else to preserve.
...
- **Ordering.** Path 1: Mongo default write first, State tag write(s) second. Path 2: the Mongo *version* write precedes State propagation, but in the existing for-comprehension the State dual-write (now carrying the tag) runs **before** the Mongo `updateDefaultVersion` — this order is deliberately kept as-is; both sides are best-effort-independent, so the default-write ordering is immaterial. In all cases the State outcome never rolls back or blocks the Mongo result.
...
- **`StateVersion` DTO** gains `tags: Option[List[String]] = None`; update the spray-json format (`jsonFormat9` → `jsonFormat10`; spray-json omits `None` fields, giving the required absent-field behaviour). `upsertVersionStateApi` gains a `tags: Option[List[String]]` parameter — the Mongo `Version` has no default flag, so the caller supplies the tags value.
- **Shared java-skip/best-effort helper.** `conditionalStateApiPropagation` is currently a `private` method on `VersionReleaseRoutes`, so `CandidateDefaultRoutes` cannot reuse it as-is. Extract the java-guard + `recoverWith`-to-`Future.unit` pattern into a shared helper (e.g. on `HttpStateApiClient` or a small shared trait) and use it from both routes — do not duplicate the guard.
- **`CandidateDefaultRoutes`** — mix in `HttpStateApiClient` (it is **not** mixed in today; its `Configuration` self-type is already satisfied by the route's existing mixins). After the successful `updateDefaultVersion`, fold in the per-platform tag propagation (guarded by the shared java skip), reusing the `versions` Seq already fetched by the route, composed into the existing `for`-comprehension without changing the `202 acceptedResponse` result. Keep the domain-service coordination free of HTTP concerns per hexagonal boundaries; the State API call lives behind the `HttpStateApiClient` port trait.
- **`VersionReleaseRoutes`** — the default-tag decision rides along with the existing `req.default` handling; the route computes `Some(List(LtsTag))` / `None` and passes it through `conditionalStateApiPropagation` into `upsertVersionStateApi`. No new branch in the response flow, only the enriched `StateVersion` payload.
